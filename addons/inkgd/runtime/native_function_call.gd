...
var BoolValue = load("res://addons/inkgd/runtime/values/bool_value.gd")
...
func call(parameters):
	...
	var coerced_type = coerced_params[0].value_type # ValueType

	if (coerced_type == ValueType.INT ||
...
func call_coerced(parameters_of_single_type):
	...
	if param_count == 2 || param_count == 1:
		...
		if _operation_funcs.has(val_type):
			...
			Utils.throw_story_exception(str("Cannot perform operation '", self.name, "' on ", val_type))
			return null
...
func call_binary_list_operation(parameters):
	...
	if ((self.name == "&&" || self.name == "||") &&
		...
		var op = _operation_funcs[ValueType.INT]
		...
		return BoolValue.new_with(result)
...
func coerce_values_to_single_type(parameters_in):
	...
	var special_case_list = null # ListValue
	...
	if val_type == ValueType.LIST:
		for val in parameters_in:
			if val.value_type == ValueType.LIST:
				...
				if item.exists:
					var casted_value = ListValue.new_with_single_item(item.result, int_val)
