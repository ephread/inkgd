...
func test_divert_to_weave_points():
	...
	assert_eq(story.continue_story_maximally(), "gather\ntest\nchoice content\ngather\nsecond time round\n")
...
func test_tunnel_vs_thread_behaviour():
	...
	assert_false(story.continue_story_maximally().find("Finished tunnel") != -1)
	assert_eq(story.current_choices.size(), 2)

	story.choose_choice_index(0)

	assert_true(story.continue_story_maximally().find("Finished tunnel") != -1)
	assert_eq(story.current_choices.size(), 3)

	story.choose_choice_index(2)

	assert_true(story.continue_story_maximally().find("Done.") != -1)


func test_tunnel_onwards_to_variable_divert_target():
	var story := InkStory.new(load_file("tunnel_onwards_to_variable_divert_target"))

	assert_eq(story.continue_story_maximally(), "This is outer\nThis is the_esc\n")
